...
def is_aligned_4k(path) -> bool:
    """Return True if *path* file size is a multiple of 4096."""
    size = os.path.getsize(path)
    return size % BLOCK_SIZE == 0


def align_image_4k():
    """Interactively pad an image file to a 4096-byte multiple."""
    while True:
        path = input(f'\n  选择要对齐的镜像（绝对路径）>> ').strip()
        if not path:
            return
    ...
    if size % BLOCK_SIZE == 0:
        print(f'  {GREEN}> 镜像已对齐 4096 字节 ({size} bytes){CLOSE}')
        return

    pad = BLOCK_SIZE - (size % BLOCK_SIZE)
    with open(path, 'ab') as f:
        f.write(b'\x00' * pad)
    new_size = size + pad
    print(f'  {GREEN}> 已写入 {pad} 字节，{size} → {new_size} bytes{CLOSE}')
    print(f'  输出: {path}')


def align_4k_check_and_error(img) -> bool:
    """Check *img* is 4096-aligned; print an error and return False if not.

    Call this before invoking avbtool on a partition image so the user
    sees a clear, actionable message instead of an opaque avbtool failure.
    """
    if not os.path.isfile(img):
        return False
    if os.path.getsize(img) % BLOCK_SIZE != 0:
        print(f'\n  {RED}ERROR！镜像未4K对齐！！！！{CLOSE}')
        print(f'  请将 {os.path.basename(img)} 对齐到 4096 字节后再签名。')
        print(f'  可在 [更多] > [镜像4096对齐] 中处理。')
        return False
    return True
